"""Merge every .md file in /handle-state/interaction-state/ into
interaction-state-TEMPORARY.md.

The index that used to live in interaction-state.md is now embedded in this
...
rewritten from external file references to internal anchors, including Obsidian
wiki links:

    [drag-drop-guide](drag-drop-guide.md)
...
A stale interaction-state-TEMPORARY.md is deleted before the merge runs, so the
file is always re-created from scratch rather than overwritten in place.

Run it from anywhere — the target directory is the one holding this script:

    python interaction-state-merge.py
"""
