"""Merge every .md file in /handle-state/communication-state/ into
communication-state-TEMPORARY.md.

The index that used to live in communication-state.md is now embedded in this
...
wiki links:
    ...
    [lwc-error-handling-guide](lwc-error-handling-guide.md)
    → [lwc-error-handling-guide](#lwc-error-handling)
...
A stale communication-state-TEMPORARY.md is deleted before the merge runs, so
...
Run it from anywhere — the target directory is the one holding this script:
    ...
    python communication-state-merge.py
"""
